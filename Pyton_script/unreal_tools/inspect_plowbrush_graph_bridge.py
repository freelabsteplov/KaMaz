...
GRAPH_NAME = "DrawPlowClearance"
OUTPUT_BASENAME = "plowbrush_drawplowclearance_graph"
...
def _write_text(path: str, text: str) -> str:
    ...
    with open(path, "w", encoding="utf-8") as handle:
        handle.write(text)
    unreal.log(f"[inspect_plowbrush_graph_bridge] Wrote file: {path}")
    return path
...
def run(output_dir: str | None = None) -> dict:
    ...
    graph_payload = json.loads(json_text)
