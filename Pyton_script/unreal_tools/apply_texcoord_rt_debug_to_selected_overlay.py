...
OUTPUT_BASENAME = "apply_texcoord_rt_debug_to_selected_overlay"
DEBUG_EMISSIVE_MULTIPLIER = 80.0


def _log(message: str) -> None:
    unreal.log(f"[apply_texcoord_rt_debug_to_selected_overlay] {message}")
...
def run(output_dir: str | None = None) -> dict:
    ...
    payload = {
        ...
        "mode": "selected_overlay_texcoord_rt_debug",
        "debug_emissive_multiplier": DEBUG_EMISSIVE_MULTIPLIER,
        "rebuild_output_path": rebuild_result.get("output_path", ""),
        "apply_result": apply_result,
        "save_result": save_result,
        "notes": [
            "Applies direct TexCoord-based SnowRT debug to the currently selected overlay actor.",
            "It does not spawn, move, rotate, or rescale any actor.",
            "Use it to validate that RT data is visible on the overlay in GUI PIE before debugging world mapping.",
        ],
    }
