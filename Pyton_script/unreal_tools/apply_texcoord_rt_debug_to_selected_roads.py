...
OUTPUT_BASENAME = "apply_texcoord_rt_debug_to_selected_roads"


def _log(message: str) -> None:
    unreal.log(f"[apply_texcoord_rt_debug_to_selected_roads] {message}")
...
def run(output_dir: str | None = None) -> dict:
    ...
    module.DEBUG_DIRECT_TEXCOORD_RT_VIS = True
    ...
    payload = {
        ...
        "mode": "texcoord_rt_debug_selected",
        "rebuild_output_path": rebuild_result.get("output_path", ""),
        "selected_apply_result": selected_apply_result,
        "save_result": save_result,
        "notes": [
            "This applies a direct TexCoord-based SnowRT visualization to the currently selected road actors.",
            "If this still shows no green, the issue is likely that the RT itself has no useful visible signal for this test rather than world-bounds mapping.",
        ],
    }
