...
OUTPUT_BASENAME = "apply_solid_debug_to_selected_roads"


def _log(message: str) -> None:
    unreal.log(f"[apply_solid_debug_to_selected_roads] {message}")
...
def run(output_dir: str | None = None) -> dict:
    ...
    module.DEBUG_FORCE_SOLID_COLOR = True
    ...
    payload = {
        ...
        "mode": "solid_debug_selected",
        "rebuild_output_path": rebuild_result.get("output_path", ""),
        "selected_apply_result": selected_apply_result,
        "save_result": save_result,
        "notes": [
            "This applies the solid-color debug receiver to the currently selected road actors.",
            "Use it to verify that the isolated receiver material is really assigned to the selected road mesh and slot 0.",
            "If the selected road does not visibly turn into the solid debug color, the issue is actor/slot targeting rather than RT visualization.",
        ],
    }
