...
OUTPUT_BASENAME = "apply_strong_visible_snow_to_selected_roads"


def _log(message: str) -> None:
    unreal.log(f"[apply_strong_visible_snow_to_selected_roads] {message}")
...
def run(output_dir: str | None = None) -> dict:
    ...
    # Temporary high-contrast visible snow tuning for selected-road validation.
    ...
    payload = {
        ...
        "mode": "strong_visible_snow_selected",
        "rebuild_output_path": rebuild_result.get("output_path", ""),
        "selected_apply_result": selected_apply_result,
        "save_result": save_result,
        "notes": [
            "This applies a stronger visible snow layer to the currently selected road actors.",
            "Use it after RT debug confirms the receiver is reading correctly but snow is still too subtle to validate visually.",
        ],
        "overrides": {
            "BaseSnowAmount": module.BASE_SNOW_AMOUNT,
            "SnowTintStrength": module.SNOW_TINT_STRENGTH,
            "SnowColorTint": list(module.SNOW_COLOR_TINT),
            "SnowUVScale": module.SNOW_UV_SCALE,
            "TraceDebugEmissiveMultiplier": module.TRACE_DEBUG_EMISSIVE_MULTIPLIER,
        },
    }
