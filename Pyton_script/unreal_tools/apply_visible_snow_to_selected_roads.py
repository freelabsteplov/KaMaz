...
OUTPUT_BASENAME = "apply_visible_snow_to_selected_roads"


def _log(message: str) -> None:
    unreal.log(f"[apply_visible_snow_to_selected_roads] {message}")
...
def run(output_dir: str | None = None) -> dict:
    ...
    payload = {
        ...
        "mode": "visible_snow_selected",
        "rebuild_output_path": rebuild_result.get("output_path", ""),
        "selected_apply_result": selected_apply_result,
        "save_result": save_result,
        "notes": [
            "This applies the normal asphalt+snow receiver to the currently selected road actors.",
            "Use it after direct RT debug confirms the selected road can read the snow render target.",
        ],
    }
